...
.run_pytest_unit:
	@$(VENV_DIR)/bin/pytest --cov=tap_kafka  --cov-fail-under=79 tests/unit -v

.run_pytest_integration:
	@TAP_KAFKA_BOOTSTRAP_SERVERS=localhost:${KAFKA_PORT} $(VENV_DIR)/bin/pytest --cov=tap_kafka  --cov-fail-under=80 tests/integration -v
